...
def get_data(filters, keys):
    ...
    for month in range(1, 13):
        ...
        for k, v in keys.items():
            ...
            if ((ktm_per_h[0]['h_total'] or 0) != 0) and ((ktm_per_h[0]['h_schicht'] or 0) != 0):
                ktm_per_schicht = ((ktm_per_h[0]['h_schicht'] or 0) / ktm_per_h[0]['h_total']) * (ktm_per_h[0]['total_ktm'] or 0)
                ktm_per_h = ktm_per_schicht / ktm_per_h[0]['h_schicht']
            else:
                ktm_per_h = 0
            data[-1][k] = ktm_per_h
    return data
